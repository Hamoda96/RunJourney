package com.hamoda.run.presntation.active_run.service
...
class ActiveRunService : Service() {
    ...
    private val baseNotification by lazy {
        NotificationCompat.Builder(applicationContext, CHANNEL_ID)
            .setSmallIcon(com.hamoda.core.presentation.designsystem.R.drawable.logo)
        ...
        when (intent?.action) {
            ACTION_START -> {
                val activityClass = intent.getStringExtra(EXTRA_ACTIVITY_CLASS)
                    ?: throw IllegalArgumentException("No activity class provider")
    ...
    private fun start(activityClass: Class<*>) {
        if (!isServiceActive) {
            ...
            val activityIntent = Intent(applicationContext, activityClass).apply {
                data = "runjourney://active_run".toUri()
